//
//  KeyedTableViewCell.m
//  Meetup
//
//  Created by Vernon Thommeret on 8/17/09.
//  Copyright 2009 Vernon Thommeret. All rights reserved.
//

#import "KeyedTableViewCell.h"
#import "State.h"

#define kKeyedRowKeyWidth		70.0
#define kKeyedRowPadding		5.0

@implementation KeyedTableViewCell

@synthesize content = _content;

- (id)initWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath reuseIdentifier:(NSString *)reuseIdentifier {
	if (self = [super initWithTableView:tableView indexPath:indexPath reuseIdentifier:reuseIdentifier]) {
		KeyedTableViewCellView *cellView = [[KeyedTableViewCellView alloc] initWithFrame:CGRectZero parentCell:self];
		[self.contentView addSubview:cellView];
		self.cellView = cellView;
		[cellView release];
	}
	return self;
}

- (void)setContent:(NSString *)content {
	[_content release];
	_content = [content copy];
	
	// redrawing the frame is expensive; only change if necessary
	CGRect newFrame = [[self class] cellFrameWithContent:self.content tableView:self.tableView indexPath:self.indexPath
										   accessoryType:self.accessoryType];
	if (self.cellView.frame.size.height != newFrame.size.height) {
		self.cellView.frame = newFrame;
		[self.cellView setNeedsDisplay];
	}
}

- (void)dealloc {
	[_content release];
    [super dealloc];
}

+ (NSString *)baseIdentifier {
	return @"Keyed";
}

+ (CGRect)cellFrameWithContent:(NSString *)content tableView:(UITableView *)tableView
					 indexPath:(NSIndexPath *)indexPath accessoryType:(UITableViewCellAccessoryType)accessoryType {	
	CGFloat insetPaddingVert = [[self class] insetOriginForTableView:tableView].y * 2.0;
	CGFloat insetWidth = [[self class] insetWidthForTableView:tableView withAccessoryType:accessoryType];
	
	insetWidth = insetWidth - kKeyedRowKeyWidth - kKeyedRowPadding;
	
	CGFloat contentHeight = [content sizeWithFont:[UIFont boldSystemFontOfSize:15.0]
								constrainedToSize:CGSizeMake(insetWidth, MAXFLOAT)
									lineBreakMode:UILineBreakModeTailTruncation].height + insetPaddingVert;
	
	return [[self class] cellFrameWithContentHeight:contentHeight tableView:tableView indexPath:indexPath];
}

@end

#pragma mark -
#pragma mark KeyedTableViewCellView Implementation

@implementation KeyedTableViewCellView

- (id)initWithFrame:(CGRect)frame parentCell:(AbstractTableViewCell *)parentCell {
	if (self = [super initWithFrame:frame parentCell:parentCell]) {
		self.fontSize = 15.0;
	}
	return self;
}

- (void)drawRect:(CGRect)rect {
	[super drawRect:rect];
	
	KeyedTableViewCell *parentCell = (KeyedTableViewCell *) self.parentCell;
	
	NSString *key = [parentCell.key lowercaseString];
	
	CGPoint insetOrigin = [[parentCell class] insetOriginForTableView:parentCell.tableView];
	CGFloat insetWidth = [[parentCell class] insetWidthForTableView:parentCell.tableView
												  withAccessoryType:parentCell.accessoryType];
	
	_highlighted ? [[UIColor whiteColor] set] : [[State sharedState].keyColor set];
	
	[key drawInRect:CGRectMake(insetOrigin.x, insetOrigin.y + 1.0, kKeyedRowKeyWidth, kKeyFontSize)
					  withFont:[UIFont boldSystemFontOfSize:kKeyFontSize]
				 lineBreakMode:UILineBreakModeTailTruncation
					 alignment:UITextAlignmentRight];
	
	insetOrigin = CGPointMake(insetOrigin.x + kKeyedRowKeyWidth + kKeyedRowPadding, insetOrigin.y);
	insetWidth = insetWidth - kKeyedRowKeyWidth - kKeyedRowPadding;
	
	_highlighted ? [[UIColor whiteColor] set] : [[UIColor blackColor] set];
	
	[parentCell.content drawInRect:CGRectMake(insetOrigin.x, insetOrigin.y, insetWidth, MAXFLOAT)
						  withFont:[UIFont boldSystemFontOfSize:self.fontSize]
					 lineBreakMode:UILineBreakModeTailTruncation];
}

@end
